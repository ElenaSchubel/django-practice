...
class UserPosts(generic.ListView):
    ...
    def get_queryset(self):
        try:
            self.post.user = User.objects.prefecth_related('posts').get(username__iexact=self.kwargs.get('username'))
        except User.DoesNotExsist:
            raise Http404
        else:
            return self.user_posts_all()
...
class CreatePost(LoginRequiredMixin,SelectionRelatedMixin,generic.CreatedView):
